import React from "react"
import { AiFillLinkedin, AiFillGithub } from "react-icons/ai"
import { motion } from "framer-motion"
import Reveal from "./Reveal"

const Contact = () => {
  return (
    <div className="px-6 max-w-[1100px] flex items-center justify-center mx-auto md:my-12" id="contact">
        <Reveal>
        <div className="place-items-center mx-auto">  
            <form
                action="https://getform.io/f/adrnxela"
                method="POST"
                className=" max-w-2xl p-5 md:p-12"
                id="form"
            >
            <p className="text-gray-100 font-bold text-[36px] mb-1">
              Let´s connect!
            </p>
            <p className="text-gray-100 text-[18px] mb-5">
              Need a website, a fresh design, or just want to bounce around some ideas? Let’s chat!
            </p>
            <input
                type="text"
                id="name"
                placeholder="Your Name ..."
                name="name"
                className="bg-white mb-2 w-full rounded-md border border-gray-600 py-2 pl-2 pr-4"
            />
            <input
              type="email"
              id="email"
              placeholder="Your Email ..."
              name="email"
              className="bg-white mb-2 w-full rounded-md border border-gray-600 py-2 pl-2 pr-4"
            />
            <textarea
              name="textarea"
              id="textarea"
              cols="30"
              rows="4"
              placeholder="Your Message ..."
              className="bg-white mb-2 w-full rounded-md border border-gray-600 py-2 pl-2 pr-4"
            />
            <button
              type="submit"
              className="w-full py-3 rounded-md text-gray-100 font-semibold text-xl bg-primary-color cursor-pointer"
            >
              Send Message
            </button>
            
          </form>

        </div>
        
        </Reveal>
    </div>
  )
}

export default Contact